...
## 11.6 RLS 적용 가이드 및 주의사항 (초안)

목적: `posts` 테이블에 대해 Row Level Security(RLS)를 활성화하고, 작성자만 자신의 글을 INSERT/UPDATE/DELETE할 수 있도록 정책을 적용합니다. 다음 절차는 안전하게 마이그레이션을 관리하고 배포 환경에서 `service_role` 키를 노출하지 않도록 하기 위한 권장 흐름입니다.

- 마이그레이션 생성 (로컬 개발 환경)
	- 예: `npx supabase migration new add_posts_rls`
	- 생성된 SQL 파일(`supabase/migrations/<timestamp>_add_posts_rls.sql`)에 아래 내용을 포함시키세요:

```sql
ALTER TABLE posts ENABLE ROW LEVEL SECURITY;

-- Everyone can read posts (optional)
...
CREATE POLICY "Authenticated users can insert their own posts" ON public.posts FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Authors can update their own posts
CREATE POLICY "Authors can update their own posts" ON public.posts FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
- 마이그레이션 적용
	- CLI로 적용하거나 CI 파이프라인에서 마이그레이션을 실행하세요. 예: `npx supabase db push` 또는 프로젝트의 마이그레이션 실행 명령을 사용합니다.

- 검증 시나리오
	- 익명(로그인 전) 사용자는 SELECT만 가능한지 확인
	- 로그인한 사용자 A가 자신의 글을 INSERT/UPDATE/DELETE 가능한지 확인
	- 사용자 A가 다른 사용자 B의 글을 UPDATE/DELETE할 수 없는지 확인
	- 실패 케이스에서 반환되는 Supabase 에러 코드를 로그에 남기고 사용자 친화적 메시지를 보여줍니다.

- 서비스 키(`service_role`) 취급 주의
	- `service_role` 키는 절대 클라이언트(브라우저)에 포함되면 안 됩니다. 서비스 키는 서버 환경변수(CI/CD, Vercel/Netlify 등)의 비밀 영역에만 저장하세요.
	- 서버 전용 API(예: `app/api/*` 라우트)에서만 `service_role`을 사용하고, 클라이언트는 anon 키(`NEXT_PUBLIC_SUPABASE_ANON_KEY`)만 사용하도록 하세요.
	- 코드베이스에서 `SUPABASE_SERVICE_ROLE` 환경변수를 직접 하드코딩하지 않았는지 검색하세요. (아래 제공된 검사 명령 참조)

- 배포 전 체크리스트
	- `supabase/migrations/<timestamp>_add_posts_rls.sql`가 커밋되어 있는가? (Git 추적)
	- 배포 환경에 `SUPABASE_SERVICE_ROLE`가 설정되어 있는가? (Vercel/Netlify/서버 설정)
	- 클라이언트 번들에 `service_role` 값이 포함되어 있지 않은가? (`git grep` 또는 빌드 아티팩트 검사)
	- 모든 API 엔드포인트가 익명 키 대신 서비스 키를 사용해야 하는지 검토했는가? 가능한 경우 최소 권한 원칙을 따르세요.

- 빠른 리포트/검사 명령
	- 레포에서 `service_role` 관련 노출 검색:

```bash
git grep -n "SUPABASE_SERVICE_ROLE" || true
git grep -n "service_role" || true
git grep -n "SUPABASE_SERVICE_KEY" || true
```

---

이 초안은 적용 전 팀/프로젝트 정책에 맞추어 조정하세요. 필요하면 제가 마이그레이션 파일 내용을 검토하거나, CI에서 안전하게 마이그레이션을 실행하는 스크립트 초안을 작성해드리겠습니다.
